...
# teste = tm.ListTimeseriesM3(filename_input = '/home/claudio/PycharmProjects/M3Data/data/M3C.xls',sheet='M3Month',\
#                   filename_output='/home/claudio/PycharmProjects/M3Data/data/M3Month.pck',forecast_period=18, frequency='Month',)
#
#
# data_sheet = pd.read_excel(filename_input, sheetname=sheet)
# data_sheet.iloc[1399]
# raw_data = data_sheet.iloc[[1399]].dropna(axis=1, how='all')
# raw_data
#
# series = list()
...
#                      frequency=frequency, start_period1=raw_data['Starting Year'].to_string(index=False, header=False),
...
# valor.get_train_data(response=False)
# pd.DataFrame({'id': [valor._id] * len(valor._serie_train),
#               'time': pd.date_range(1, periods=len(valor._serie_train),
#                                     freq=valor._foptions[valor._frequency]),
#               'value': valor._serie_train})
#
...
times_series_M3Month= pickle.load(open('/home/claudio/PycharmProjects/M3Data/data/M3Month.pck', "rb"))



for i in range(len(times_series_M3Month)):
    times_series_M3Month[i] = process_times_series(times_series_M3Month[i])
    tm.ListTimeseriesM3.save(listObject=times_series_M3Month,filename_output='/home/claudio/PycharmProjects/M3Data/data/M3Month_pred_wt_constant.pck')
...
#Year Data with 6 to forecast
length_moving = 19
timesseries_year = pickle.load(open('/home/claudio/PycharmProjects/M3Data/data/M3C_pred_monthly_final_20180508.pck', "rb"))
...
for k,ts in enumerate(timesseries_year):
    ...
    for i in range(len(pred)):
        sMape[k][i] = abs(real[i] - pred[i]) / (
                ((real[i]) + (pred[i])) / 2) * 100
        if i==2 and sMape[k][i] <= 0:
            sMape[k][i] = 0
#TODO:Verify negative values
...
describe(np.concatenate((sMape[:, 0],sMape[:, 1],sMape[:, 2],sMape[:, 3],sMape[:, 4],sMape[:, 5],sMape[:, 6],sMape[:, 7],sMape[:, 8],sMape[:, 9],sMape[:, 10],sMape[:, 11])))
describe(np.concatenate((sMape[:, 0],sMape[:, 1],sMape[:, 2],sMape[:, 3],sMape[:, 4],sMape[:, 5],sMape[:, 6],sMape[:, 7],sMape[:, 8],sMape[:, 9],sMape[:, 10],sMape[:, 11],sMape[:, 12],sMape[:, 13],sMape[:, 14])))
describe(np.concatenate((sMape[:, 0],sMape[:, 1],sMape[:, 2],sMape[:, 3],sMape[:, 4],sMape[:, 5],sMape[:, 6],sMape[:, 7],sMape[:, 8],sMape[:, 9],sMape[:, 10],sMape[:, 11],sMape[:, 12],sMape[:, 13],sMape[:, 14],sMape[:, 15],sMape[:, 16],sMape[:, 17])))
...
sMape_df_stacked['col'] = np.where(sMape_df_stacked['Forecast']<=9,1,2)
g = sns.FacetGrid(sMape_df_stacked,col="Forecast",col_wrap=6)
